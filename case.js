...
// =============== MAIN FUNCTION ===============
module.exports = async function handleCommand(ridzcoder, m, command, isGroup, isAdmin, groupAdmins,isBotAdmins,groupMeta,config) {
    ...
    const from = ridzcoder.decodeJid(m.key.remoteJid);
    const sender = m.key.participant || m.key.remoteJid;
    const participant = ridzcoder.decodeJid(m.key.participant || from);
...
const isOwner = senderJid === botNumber;
    const reply = (text) => ridzcoder.sendMessage(from, { text: stylishReply(text) }, { quoted: m });

    const ctx = m.message.extendedTextMessage?.contextInfo || {};
    const quoted = ctx.quotedMessage;
    const quotedSender = ridzcoder.decodeJid(ctx.participant || from);
    const mentioned = ctx.mentionedJid?.map(ridzcoder.decodeJid) || [];
...
if (isGroup && global.antilink && global.antilink[from]?.enabled) {
    ...
    if (linkPattern.test(bodyText)) {
        ...
        const groupMeta = await ridzcoder.groupMetadata(from);
        ...
        if (!isSenderAdmin && isBotAdmin) {
            try {
                await ridzcoder.sendMessage(from, { delete: m.key });
                ...
                if (settings.mode === "kick") {
                    await ridzcoder.groupParticipantsUpdate(from, [sender], "remove");
                }
            } catch (err) {
    ...
    if (mentionedUsers.length > 0) {
        if (!isSenderAdmin && isBotAdmin) {
            try {
                ...
                // 🚷 If mode is "kick"
                if (settings.mode === "kick") {
                    await ridzcoder.groupParticipantsUpdate(from, [m.sender], "remove");
                }
            } catch (err) {
...
╰═════Rɪᴅᴢ Cᴏᴅᴇʀ❦══════❏`;
    ...
    const menuImage = "https://files.catbox.moe/ld87wq.png";

    try {
        await ridzcoder.sendMessage(from, {
            image: { url: menuImage },
            caption: stylishReply(pongText),
            contextInfo: {
                forwardingScore: 999,
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: "120363404529319592@newsletter",
                    newsletterName: "Vermilion MD Updates 🔥",
                    serverMessageId: 1
                }
            }
        }, { quoted: m });
        ...
        console.error('Ping image failed:', err);
        await reply(pongText);
    }
    break;
}
...
╰═════Rɪᴅᴢ Cᴏᴅᴇʀ❦══════❏`;
    ...
    try {
        ...
        if (data.cod !== 200) return reply("❌ Unable to find that location. Please check spelling.");

        const weatherText = `
╭════〔VERMILION MD WEATHER〕═══❏
│┃➥ 🌤️ Weather for ${data.name}
...
╰═════Rɪᴅᴢ Cᴏᴅᴇʀ❦══════❏`;
        ...
        const weatherImage = "https://files.catbox.moe/ld87wq.png";

        await ridzcoder.sendMessage(from, {
            image: { url: weatherImage },
            caption: stylishReply(weatherText),
            contextInfo: {
                forwardingScore: 999,
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
        ...
        reply("❌ Unable to retrieve weather info.");
    }
    break;
}
...
case 'time': {
    try {
        if (!text) return reply("🌍 Please provide a city or country name.");
...
╰═════Rɪᴅᴢ Cᴏᴅᴇʀ❦══════❏`;
        ...
        const timeImage = "https://files.catbox.moe/ld87wq.png";

        await ridzcoder.sendMessage(from, {
            image: { url: timeImage },
            caption: stylishReply(timeText),
            contextInfo: {
                forwardingScore: 999,
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
        ...
        reply("❌ Unable to fetch time data.");
    }
    break;
}
            // ================= GITCLONE =================
            case 'gitclone': {
                try {
                    ...
                    await ridzcoder.sendMessage(from, { document: { url: zipUrl }, fileName: filename, mimetype: 'application/zip' }, { quoted: m });
            ...
            case 'save': {
                try {
                    ...
                    const mediaBuffer = await ridzcoder.downloadMediaMessage(quoted);
                    ...
                    await ridzcoder.sendMessage(m.sender, payload, { quoted: m });
            ...
            case 'igdl': {
                ...
                try {
                    ...
                    const progressMsg = await ridzcoder.sendMessage(from, { text: stylishReply("⏳ Fetching media... Please wait!") }, { quoted: m });
                    ...
                    if (res.error) {
                        await ridzcoder.sendMessage(from, { react: { text: "❌", key: m.key } });
                        return reply(`⚠️ Error: ${res.error}`);
                    }

                    await ridzcoder.sendMessage(from, { text: stylishReply("⏳ Media found! Downloading now...") }, { quoted: m });

                    if (res.type === "video") {
                        await ridzcoder.sendMessage(from, { video: { url: res.media }, caption: stylishReply(`✅ Downloaded video from ${res.platform}!`) }, { quoted: m });
                    } else if (res.type === "image") {
                        await ridzcoder.sendMessage(from, { image: { url: res.media }, caption: stylishReply(`✅ Downloaded photo from ${res.platform}!`) }, { quoted: m });
                    }

                    await ridzcoder.sendMessage(from, { text: stylishReply("✅ Done!") }, { quoted: m });
                ...
                try {
                    ...
                    if (json.images && json.images.length > 0) {
                        for (const imgUrl of json.images) {
                            await ridzcoder.sendMessage(from, { image: { url: imgUrl } }, { quoted: m });
                        }
                    } else {
                        await ridzcoder.sendMessage(from, { video: { url: json.play }, mimetype: 'video/mp4', caption: stylishReply(caption) }, { quoted: m });
                        setTimeout(async () => {
                            await ridzcoder.sendMessage(from, { audio: { url: json.music }, mimetype: 'audio/mpeg' }, { quoted: m });
                        }, 3000);
                    }
                } catch (err) {
    ...
    break;
            ...
            case 'play': {
                try {
                    ...
                    await ridzcoder.sendMessage(from, { text: stylishReply(`🎶 Playing *${apiData.result.title || video.title}* 🎧`) }, { quoted: m });
                    await ridzcoder.sendMessage(from, { audio: { url: filePath }, mimetype: "audio/mpeg", fileName: `${(apiData.result.title || video.title).substring(0, 100)}.mp3` }, { quoted: m });
...
case 'toaudio': {
    try {
        ...
        await ridzcoder.sendMessage(from, { audio: audioBuffer, mimetype: 'audio/mpeg', ptt: false }, { quoted: m });
...
case 'tovoicenote': {
    try {
        ...
        // ✅ Send voice note
        const audioBuffer = fs.readFileSync(outputPath);
        await ridzcoder.sendMessage(from, { audio: audioBuffer, mimetype: 'audio/ogg', ptt: true }, { quoted: m });
...
case 'add': {
    ...
    try {
        const res = await ridzcoder.groupParticipantsUpdate(from, [numbersOnly], 'add');
        for (let i of res) {
            const invv = await ridzcoder.groupInviteCode(from);
...
case 'hidetag': {
    ...
    try {
        ...
        const text = args.join(' ');
        await ridzcoder.sendMessage(from, { text, mentions: participants });
    } catch (err) {
...
case 'everyone':
    if (!isGroup) {
        return await ridzcoder.sendMessage(from, { text: '❌ This command can only be used in groups!' });
    }
...
case 'remove': {
    ...
    // 🛡️ Protect owner & bot
    const botNumber = ridzcoder.user?.id || '';
    ...
    try {
        ...
        const result = await Promise.race([
            ridzcoder.groupParticipantsUpdate(from, [target], 'remove'),
...
case 'promote': {
    try {
        ...
        const groupMetadata = await ridzcoder.groupMetadata(m.chat);
        ...
        const botNumber = ridzcoder.user.id.replace(/[^0-9]/g, '');
...
case 'demote': {
    try {
        ...
        const botJid = ridzcoder.user.id;
...
case 'kayiza': {
    try {
        ...
        console.error("kayiza command error:", err);
...
fs.watchFile(file, () => {
    ...
    try { 
        ...
        console.error(`${colors.bgGreen}${colors.yellow}❌ Error reloading main file:${colors.reset}`, err);
    }
});